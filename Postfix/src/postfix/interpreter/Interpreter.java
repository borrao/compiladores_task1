...
public class Interpreter implements Expr.Visitor<Integer> {
	...
	@Override
	public Integer visitNumberExpr(Expr.Number expr) throws RuntimeException{
		int n;
		try{
			n = Integer.parseInt(expr.value);
			return n;
		}catch(Exception e) {
			String en = env.get(expr.value);
			if(en != null){
				return visitNumberExpr(new Expr.Number(en));
			}else {
				throw new RuntimeException(expr.value+" cannot be resolved.");
			}
		}
	}
